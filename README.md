...
最初は[読み取り専用の試行指示](prompts/read-only-trial.md)で結果と監査範囲を確認し、GitHub連携では対象リポジトリと許可する操作を必要な範囲に絞ってください。標準ポリシーにはIssue・PRの作成と、厳しい条件付きの自動マージが含まれます。ChatGPTの実行結果や生成した変更には誤りがあり得ます。検出結果、権限、PRの差分と実行レポートを確認したうえで、ご自身の判断で利用してください。Model Radarは検出の正確性、変更の安全性、継続的な実行を保証しません。
